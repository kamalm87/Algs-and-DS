...
struct LinkedList{
	...
	struct Node{
		NodeT Data;
		Node<NodeT> *Next, *Prev;
		Node(NodeT d = NodeT(), Node<NodeT> *n = nullptr, Node<NodeT> *p = nullptr) : Data(d), Next(n), Prev(p){}
		...
		Node<NodeT>* operator=(const Node<T> &rhs){
			Node<T> *n = new Node<T>();
			n->Data = rhs.Data;
			n->Next = rhs.Next;
			n->Prev = rhs.Prev;

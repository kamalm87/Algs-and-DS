...
public:
	...
	Heap(DynamicArray<T> &A, HeapType::HeapType type){
	...
	Heap(T* a, size_t n, HeapType::HeapType type = HeapType::Max) : Capacity(n), Size(n), A(a), Type(type){
		Build();
		HeapSize = n;
	}
	...
	// NOTE:
	// Assumes that the generic type T is a 'complex' type with a modifiable sort value
	// Specialize this for immutable types for optimization! 
	bool Contains(T d){
